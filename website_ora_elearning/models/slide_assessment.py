...
class ORAResponse(models.Model):
    ...
    can_resubmit = fields.Boolean("Can Resubmit Response")
    ...
    def action_mark_assessed(self):
        if self.state == 'submitted':
            if any(line.assess_type == 'staff' for line in self.slide_rubric_staff_line):
                self.state = 'assessed'
...
class OpenResponseUserLine(models.Model):
    ...
    response_id = fields.Many2one('ora.response')
...
class OpenResponseRubricAssess(models.Model):
    ...
    response_assess_id = fields.Many2one('open.response.rubric.staff')
